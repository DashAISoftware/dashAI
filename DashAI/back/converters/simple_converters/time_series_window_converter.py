...
class TimeSeriesWindowConverter(BaseConverter):
    ...
    SHORT_DESCRIPTION = "Converts time series to regression with lag features and future targets."
    ...
    def fit(self, x: DashAIDataset, y: Union[DashAIDataset, None] = None) -> "TimeSeriesWindowConverter":
        """
        ...
        if len(x) < min_required_rows:
            raise ValueError(
                f"Dataset has {len(x)} rows but needs at least {min_required_rows} rows "
                f"(window_size={self.window_size} + horizon={self.horizon})"
            )
    ...
    def transform(self, x: DashAIDataset, y: Union[DashAIDataset, None] = None) -> DashAIDataset:
        """
        ...
        if self._target_column_validated not in df.columns:
            raise ValueError(
                f"Target column '{self._target_column_validated}' not found in transform dataset"
            )
        ...
        target_series = df[self._target_column_validated]
        
        for lag in range(1, self.window_size + 1):
            lag_column_name = f"lag_{lag}"
            result_df[lag_column_name] = target_series.shift(lag)
        
        # Create target column (shifted h steps into the future)
        target_column_name = f"y_target_{self.horizon}"
        result_df[target_column_name] = target_series.shift(-self.horizon)
        
        # Include any other columns that are not the target column
        # This preserves potential date columns or other features
        other_columns = [col for col in df.columns if col != self._target_column_validated]
