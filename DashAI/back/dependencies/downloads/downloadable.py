...
class HFDownloadableMixin(DownloadableMixin):
    ...
    HF_REPOS: List[Tuple[str, str]] = []

    @classmethod
    def hf_repos(cls) -> List[Tuple[str, str]]:
        """Return the ``(repo_id, repo_type)`` pairs this component needs."""
    ...
    def download(cls, report: Optional[ProgressReporter] = None) -> None:
        for rid, rtype in cls.hf_repos():
            ...
            snapshot_download(repo_id=rid, repo_type=rtype, local_dir=str(target))
