...
class DiceCounterfactual(BaseLocalExplainer):
    ...
    def _summarize_instance(self, instance: dict, metadata: dict) -> str:
        """Build the DiCE counterfactual comparison sentence for one instance.
        ...
        for cf_idx, counterfactual in enumerate(counterfactuals):
            ...
            lines.append(
                f"Counterfactual {cf_idx + 1}: changing {changed} yields {cf_name}."
            )
        return "\n".join(lines)
    ...
    def story(self, explainer_output, prediction_context):
        """Build the DiCE counterfactual sentence from ``self.explanation``.
