...
class PartialDependence(BaseGlobalExplainer):
    ...
    def _create_plot(self, data: List[object]) -> List[Artifact]:
        """Helper method to create the explanation plots using plotly.

        Emits one plotly artifact per feature and class curve, titled by that
        curve, so the frontend lists them in its instance selector instead of
        ...
        -------
        List[Artifact]
            One plotly artifact per feature and class curve.
        """
        # Lazy imports
        ...
        for df in data:
            ...
            artifacts.append(PlotlyArtifact(payload=fig, title=column_name))
