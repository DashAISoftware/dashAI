...
def _rebuild_dataset_with_transformed_columns(
    ...
    """
    Replaces specific columns in the base dataset with columns from the transformed dataset,
    preserving their original positions. Also appends any additional columns that were
    generated by the transformer at the end. Keeps the features and metadata consistent.
    ...
    transformed : DashAIDataset
        The dataset resulting from applying a transformer, containing updated and/or new columns.
    ...
    -------
    DashAIDataset
        A new dataset with the specified columns replaced in-place, new columns appended,
        and original metadata and split information preserved.
    """    
    ...
    new_dataset._data = update_metadata_with_features(new_dataset._data, new_dataset.features)
    ...
    def run(
        ...
        try:
            ...
            for converter_info in converter_instances:
                ...
                # Now we need to merge the transformed data back into the original dataset, preserving their original positions
                loaded_dataset = _rebuild_dataset_with_transformed_columns(loaded_dataset, transformed_dataset, scope_column_names, scope_column_indexes)
