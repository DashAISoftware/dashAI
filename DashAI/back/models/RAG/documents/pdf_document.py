...
class PDFDocument(BaseDocument):
    ...
    def get_text(self) -> str:
        if self.PARSER == "PyPDF2":
            ...
            import textract, re
            try:
                text = textract.process(self.file_path, output_encoding='utf-8').decode('utf-8')
                def limpiar_texto(texto):
                    ...
                    texto = re.sub(r'[\x00-\x1f\x7f]', ' ', texto)  # Reemplaza por espacio
                ...
                raise ValueError(f"Error extracting text from PDF file {self.file_path}: {str(e)}")
        else:
