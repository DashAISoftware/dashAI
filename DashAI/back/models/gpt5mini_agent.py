...
class Gpt5MiniAgentSchema(BaseSchema, AgentSchema, OpenAIAPISchema):
    """Schema for GPT-5-mini agent hyperparameters.

    Configures the generation behaviour of the GPT-5-mini agent, including
...
class Gpt5MiniAgent(AgenticModel):
    """GPT-5-mini model for agentic text generation using OpenAI.

    GPT-5-mini is a lightweight variant of OpenAI's GPT-5 family, optimized
    for fast inference, efficient reasoning, and reliable tool use. This
    integration enables persistent conversations, automatic context
    summarization, and seamless interaction with external tools through the
    LangChain agent framework.
    """

    COMPATIBLE_COMPONENTS = ["AgentTask"]
    SCHEMA = Gpt5MiniAgentSchema
    METADATA = {"family_model": "OpenAI"}
    DISPLAY_NAME: str = MultilingualString(
        en="GPT-5-mini model", es="Modelo GPT-5-mini"
    )
    ...
    def __init__(self, **kwargs):
        """Initialise a GPT-5-mini agent backed by the OpenAI Chat API.
