...
Your plugin package **must** use the `dashai-` prefix (e.g. `dashai-my-model`) for
DashAI to automatically discover it when installed. More community plugins:
[pypi.org/search/?q=dashai](https://pypi.org/search/?q=dashai).
:::
...
1. **Review other plugin examples**
   ...
   The [Plugin Overview](./overview) page walks through a complete working plugin
   (`dashai-phi-model-package`). More examples:
   [pypi.org/search/?q=dashai](https://pypi.org/search/?q=dashai).
