...
for example, to understand how the model responds to a particular combination of features,
or to demonstrate the model's behavior in a classroom or presentation context.
...
| Manual prediction form is missing columns | Model configuration has changed since training          | Re-open the model and verify the input columns in the session setup              |
| Download button produces an empty file    | Prediction job did not complete successfully            | Check the Job Queue for errors and re-run the prediction                         |
