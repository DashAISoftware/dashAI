...
This opens the upload flow inline — you stay on the same screen throughout the entire process.
...
define custom **NA values**, both sets are combined — your custom strings are added
...
- If a numeric column appears as `Categorical`, it likely contains non-numeric characters
  (spaces, symbols, or mixed values) — check the source file and re-upload after cleaning.
...
| A numeric column shows as `Categorical`   | Column contains non-numeric characters | Fix the source file and re-upload                          |
