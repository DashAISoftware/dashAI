...
class TestPromptCRUD:
    ...
    def test_create_duplicate_prompt_fails(self, client: TestClient):
        ...
        # The TestClient re-raises unhandled exceptions from the app, so the
        # IntegrityError on the UNIQUE constraint propagates directly.
        payload2 = {
            "class_name": "CustomRAGGenerationPrompt",
            "name": "Dup Prompt 2",
            "parameters": {"template": template},
        ...
        with pytest.raises(sa_exc.IntegrityError):
            client.post("/api/v1/prompt/", json=payload2)
...
class TestPromptSessionIntegration:
    ...
    def test_session_rejects_invalid_prompt_class(self, client: TestClient, test_doc_id: int):
        ...
        # Session accepts the payload; component resolution happens later at runtime
        stored = _post_and_get(client, params)
        prompt = stored["parameters"]["prompt"]
        assert prompt["component"] == "NonExistentPrompt", (
            "Unknown component should be stored as-is"
        )
