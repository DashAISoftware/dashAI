"""Unit tests for the concrete evaluation diagnostics."""
...
from DashAI.back.diagnostics.base_diagnostic import (
    BaseDiagnostic,
    DiagnosticError,
    as_labels,
    resolve_class_names,
...
def test_roc_curve_refuses_hard_labels():
    with pytest.raises(DiagnosticError, match="class probabilities"):
...
def test_roc_curve_refuses_a_single_class_split():
    ...
    with pytest.raises(DiagnosticError, match="single class"):
...
def test_classification_report_has_a_row_per_class_and_averages(classification_data):
    y_true, probabilities = classification_data

    artifacts = ClassificationReport().compute(y_true, probabilities, CLASS_NAMES)
...
def test_every_diagnostic_output_normalizes(classification_data, regression_data):
    y_true, probabilities = classification_data
    ...
    outputs = [
        ...
        ClassificationReport().compute(y_true, probabilities, CLASS_NAMES),
...
def test_diagnostics_declare_their_probability_requirement():
...
def test_diagnostics_are_registered_under_one_type():
    """The registry keys every diagnostic under the same component type."""
    for diagnostic in (
        ConfusionMatrix,
        RocCurve,
        PrecisionRecallCurve,
        ...
        assert issubclass(diagnostic, BaseDiagnostic)
        assert diagnostic.COMPATIBLE_COMPONENTS


def test_a_diagnostic_never_receives_model_inputs():
    """Diagnostics compare predictions against the truth and nothing else.

    Taking X would make it an explainer, so the contract must not offer one.
    """
    import inspect

    parameters = inspect.signature(BaseDiagnostic.compute).parameters
