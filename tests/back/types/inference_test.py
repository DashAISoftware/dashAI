...
def test_inference_consistency(client: TestClient):
    ...
    inferred_csv = _infer(
        client, csv_path, "text/csv",
        {"separator": ",", "methods": ["DashAIPtype"]}
    )
    inferred_json = _infer(
        client, json_path, "application/json",
...
def test_dummy_different_from_dashaiptype():
    ...
    inferred_ptype = infer_types(df, method="DashAIPtype")
    inferred_dummy = infer_types(df, method="Dummy")

    assert inferred_ptype["few_int"]["type"] != inferred_dummy["few_int"]["type"]
    

    assert inferred_ptype["float_comma"]["type"] != inferred_dummy["float_comma"]["type"]
